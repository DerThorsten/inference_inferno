...
static void 
PottsBenchmarkInferno(benchmark::State& state) {
    ...
    inferno::models::GeneralDiscreteModel model(nVar, nLabels);
    ...
    while (state.KeepRunning()){
        ...
        state.PauseTiming();
        for( int i = 0; i < model.nVariables(); ++i ) {
            labels[i] = rand() % nLabels ;
        }
        state.ResumeTiming();
...
static void PottsBenchmarkInfernoMem(benchmark::State& state) {
    ...
    inferno::models::GeneralDiscreteModel model(nVar, nLabels, false);
...
static void PottsBenchmarkOpengm(benchmark::State& state) {
    ...
    while (state.KeepRunning()){
        state.PauseTiming();
        for( int i = 0; i < gm.numberOfVariables(); ++i ) {
            labels[i] = rand() % nLabels ;
        }
        state.ResumeTiming();
